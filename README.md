...
The google takeout output changes all the time. Files change names, products are added/removed, fields on the data change, the types or accuracy of data changes. I have been using and updating this parser since 2020, and other users have successfully parsed data back from 2018, but the data in your account may hit an edge case that we never happened to. If you do hit an error, feel free to [create an issue](https://github.com/purarue/google_takeout_parser/issues/new?title=broken+parser:).
